...
bool CryptoSign::rsaEncrypt(const std::string &message, std::string *signature) {
    signature->clear();
    ...
    if (!EVP_SignUpdate(mdCtx, message.data(), message.size())) {//计算摘要
        LOG(ERROR) << "phase 2 error";
        return false;
    }
    if (!EVP_SignFinal(mdCtx, reinterpret_cast<unsigned char *>(signature->data()), &signatureLength, pivKey)) {//终止摘要计算,
        LOG(ERROR) << "phase 3 error";
        return false;
    }
    signature->resize(signatureLength);
    //printDigest(*signature);
    return true;
}
